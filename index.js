...
app.post('/api/signup', (req, res) => {
    const { email, password, mobile, role = "registered" } = req.body;
    ...
    const newUser = { email, password: hashedPassword, mobile, role, following: [], verified: true };
    users.push(newUser);
...
app.post('/api/login', (req, res) => {
    ...
    if (user && bcrypt.compareSync(password, user.password)) {
        const token = jwt.sign({ email: user.email, mobile: user.mobile, role: user.role }, 'your_jwt_secret', { expiresIn: '1h' });
...
app.post('/api/posts', authenticateJWT, (req, res) => {
    ...
    const newPost = { id: posts.length + 1, title, description, author: req.user.email, comments: [], likes: 0 };
    posts.push(newPost);
...
// Delete a comment
app.delete('/api/posts/:postId/comments/:commentId', authenticateJWT, (req, res) => {
    ...
    if (comment.author !== req.user.email && req.user.role !== 'admin' && post.author !== req.user.email) {
...
app.delete('/api/posts/:id', authenticateJWT, authorizeRole(['admin']), (req, res) => {
    const { id } = req.params;
    const posts = readJSONFile(POSTS_FILE);
